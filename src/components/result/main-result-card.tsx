...
export function MainResultCard({ result }: MainResultCardProps) {
  ...
  // Usar viabilityScore se disponível, caso contrário, inferir do tipo de resultado
    ...
    >
      <AdvancedCard className={`${result.result.bgColor} border-2 border-opacity-20 dark:border-opacity-30 shadow-2xl`}>
        <div className="p-8">
          ...
          {/* Barra de Recomendação - exibir apenas para resultados válidos */}
          ...
          {result.result.type === 'excessive_use' && result.testCount && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.8 }}
              className="bg-white dark:bg-gray-800 bg-opacity-50 dark:bg-opacity-50 rounded-lg p-4"
            >
